...
As colunas `tipo`, `fiscal_email`, `gestor_email` e `status` sao opcionais. Quando `tipo` nao vier preenchido, o sistema assume `contrato`. Para atas, use `ata_registro_preco`. Datas aceitam `YYYY-MM-DD`, `DD/MM/YYYY` ou `DD-MM-YYYY`.
