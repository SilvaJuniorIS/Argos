...
def test_import_contracts_syncs_to_operational_contratos(
    tmp_path,
    db_session: Session,
) -> None:
    path = tmp_path / "contratos.xlsx"
    ...
    result = import_contracts(path, db_session)

    assert result["importados"] == 1
    assert result["sincronizados"] == 1
    contrato = db_session.query(Contrato).one()
    assert contrato.numero == "123/2026"
    assert contrato.secretaria.nome == "Saude"
    assert contrato.fornecedor.cnpj == "21091888000195"
    assert contrato.termino == date(2026, 5, 23)
