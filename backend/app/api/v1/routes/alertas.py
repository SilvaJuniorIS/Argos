...
def list_alertas(
    ...
    return alerta_service.list_alertas_api(
        db,
...
def gerar_alertas(
    ...
    resultado = alerta_service.gerar_alertas_operacionais(db)
    contratos_importados = check_contract_expiration(db)
    resultado["contratos_importados"] = contratos_importados
    resultado["total"] += contratos_importados
...
def marcar_lido(
    ...
    try:
        alerta = alerta_service.marcar_como_lido(
            db,
            alerta_id,
            current_user.id,
...
def resolver_alerta(
    ...
    try:
        alerta = alerta_service.resolver_alerta(
            db,
            alerta_id,
            current_user.id,
            scoped_secretaria_ids,
