...
from app.services import contract_attachment_service
...
def _filtered_stmt(
    *,
    ...
    conditions = []
    if tipo_instrumento:
        conditions.append(Contract.tipo_instrumento == tipo_instrumento)
    if vencendo_em_30:
    ...
    if status_filter:
        conditions.append(func.lower(Contract.status) == status_filter.lower())
    if fornecedor:
        term = f"%{fornecedor}%"
...
@router.get("/{contract_id}/documentos", response_model=list[ContractAttachmentOut])
def list_contract_documents(
    contract_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    _: Annotated[User, Depends(get_current_user)],
) -> list[ContractAttachmentOut]:
    try:
        return contract_attachment_service.list_by_contract(db, contract_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc


@router.post(
    "/{contract_id}/documentos",
...
async def upload_contract_document(
    ...
    try:
        return await contract_attachment_service.upload(db, contract_id, file, tipo, versao, current_user)
    except ValueError as exc:
...
@router.get("/documentos/{attachment_id}/download")
def download_contract_document(
    ...
    try:
        attachment = contract_attachment_service.get_attachment(db, attachment_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    path = storage.resolve_path(attachment.caminho_storage)
    if not path.is_file():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Arquivo nao encontrado")
    return FileResponse(path, filename=attachment.nome_arquivo, media_type=attachment.content_type)


@router.delete("/documentos/{attachment_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_contract_document(
    ...
    try:
        contract_attachment_service.delete(db, attachment_id, current_user)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
def contracts_dashboard(
    db: Annotated[Session, Depends(get_db)],
    _: Annotated[User, Depends(get_current_user)],
    tipo_instrumento: Annotated[str | None, Query(max_length=40)] = None,
) -> ContractDashboard:
    base_conditions = [Contract.tipo_instrumento == tipo_instrumento] if tipo_instrumento else []
    ...
    vencidos = select(func.count(Contract.id)).where(
        *base_conditions, Contract.dias_para_vencimento < 0
    )
    ativos = select(func.count(Contract.id)).where(
        *base_conditions,
        func.lower(Contract.status) == "ativo",
    )
    valor = select(func.coalesce(func.sum(Contract.valor_total), 0)).where(*base_conditions)
...
def update_contract(
    ...
    tipo = updates.get("tipo_instrumento", contract.tipo_instrumento)
    numero = updates.get("numero_contrato", contract.numero_contrato)
    if numero:
        duplicate = db.scalar(
            select(Contract.id).where(
                Contract.tipo_instrumento == tipo,
                Contract.numero_contrato == numero,
        ...
        if duplicate:
            raise HTTPException(
                ...
                detail="Ja existe instrumento deste tipo com este numero",
            )
