...
def _apply_filtros(stmt, filtros: ContratoFiltros, today: date):
    if filtros.tipo_instrumento:
        stmt = stmt.where(Contrato.tipo_instrumento == filtros.tipo_instrumento)
    if filtros.numero:
...
def get_by_tipo_numero(
    ...
    *,
    ...
    stmt = select(Contrato).where(
        Contrato.tipo_instrumento == tipo_instrumento,
        Contrato.numero == numero,
    )
    if exclude_id is not None:
        stmt = stmt.where(Contrato.id != exclude_id)
    return db.scalar(stmt)
