...
function expirationClass(days?: number | null) {
  ...
  if (days <= 15) return 'bg-orange-50 text-orange-700'
...
export default function Contratos() {
  ...
  const tipoInstrumento = pathname.startsWith('/atas') ? 'ata_registro_preco' : 'contrato'
  const isAta = tipoInstrumento === 'ata_registro_preco'
  ...
  }, [page, status, secretaria, fornecedor, vencendo30, tipoInstrumento])
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold">
            {isAta ? 'Atas de registro de precos' : 'Contratos'}
          </h1>
          <div className="mt-2 flex gap-2 text-sm">
            <Link className={!isAta ? 'text-[#1D4ED8]' : 'text-slate-600'} to="/contratos">
              Contratos
            </Link>
            <span className="text-slate-600">|</span>
            <Link className={isAta ? 'text-[#1D4ED8]' : 'text-slate-600'} to="/atas">
              Atas
            </Link>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <Card title={isAta ? 'Atas ativas' : 'Contratos ativos'} value={dashboard?.contratos_ativos ?? 0} />
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200">
        <table className="w-full min-w-[900px] text-sm">
          ...
          <tbody>
            {items.map((contract) => (
              <tr key={contract.id} className="border-t border-slate-200 bg-white hover:bg-slate-50">
                <td className="px-4 py-3">
                  <Link to={`/contratos/${contract.id}`} className="text-[#1D4ED8] hover:underline">
                ...
                <td className="px-4 py-3 text-slate-700">{contract.fim_vigencia}</td>
                <td className="px-4 py-3">
      ...
      <div className="flex items-center justify-between text-sm text-slate-600">
        <span>{loading ? 'Carregando...' : `${total} ${isAta ? 'atas' : 'contratos'} encontrados`}</span>
        <div className="flex items-center gap-2">
          <button className="rounded-lg border border-slate-300 bg-white px-3 py-1 disabled:opacity-40" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>
            Anterior
          </button>
          <span>Pagina {page} de {pages}</span>
          <button className="rounded-lg border border-slate-300 bg-white px-3 py-1 disabled:opacity-40" disabled={page >= pages} onClick={() => setPage((p) => p + 1)}>
            Proxima
          </button>
        </div>
      </div>
