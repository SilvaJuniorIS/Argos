...
export default function ContratoForm() {
  ...
  const [secretariaEmailAlertas, setSecretariaEmailAlertas] = useState('')
    ...
    async function load() {
      ...
      try {
        ...
        if (contratoId) {
          ...
          const secretaria = secs.find((s) => s.id === contrato.secretaria_id)
          setForm({
            ...
            tipo_instrumento: contrato.tipo_instrumento || 'contrato',
            fiscal_responsavel_id: contrato.fiscal_responsavel_id || null,
            gestor_responsavel_id: contrato.gestor_responsavel_id || null,
          })
          setSecretariaEmailAlertas(secretaria?.email_alertas || '')
        } else {
          const secretariaId = user?.secretaria_id || secs[0]?.id || 0
          const secretaria = secs.find((s) => s.id === secretariaId)
          setForm((current) => ({
            ...current,
  ...
  function updateField(field: keyof ContratoPayload, value: string | number | null) {
    if (field === 'secretaria_id') {
      const secretaria = secretarias.find((s) => s.id === value)
      setSecretariaEmailAlertas(secretaria?.email_alertas || '')
    }
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSaving(true)
    try {
      const secretaria = secretarias.find((s) => s.id === form.secretaria_id)
      if (secretaria && (secretaria.email_alertas || '') !== secretariaEmailAlertas.trim()) {
        await updateSecretaria(secretaria.id, {
          email_alertas: secretariaEmailAlertas.trim() || null,
        })
      }
      const payload = {
        ...form,
  ...
  if (loading) return <p className="text-slate-600">Carregando formulario...</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={onSubmit} className="space-y-5 rounded-lg border border-slate-200 bg-white p-5">
        <div className="grid gap-4 md:grid-cols-3">
          <Field label="Tipo">
            <select
              className="input"
              value={form.tipo_instrumento}
              onChange={(e) => updateField('tipo_instrumento', e.target.value)}
            >
              <option value="contrato">Contrato</option>
              <option value="ata_registro_preco">Ata de registro de precos</option>
            </select>
          </Field>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="E-mails de alerta da secretaria">
            <input
              className="input"
              placeholder="contratos@secretaria.gov.br; gabinete@secretaria.gov.br"
              value={secretariaEmailAlertas}
              onChange={(e) => setSecretariaEmailAlertas(e.target.value)}
            />
          </Field>
...
function Field({ label, invalid = false, children }: { label: string; invalid?: boolean; children: React.ReactNode }) {
  return (
    <label className="block text-sm">
      ...
      {children}
      {invalid && <span className="mt-1 block text-xs text-red-600">Campo obrigatorio ou invalido.</span>}
    </label>
  )
}
