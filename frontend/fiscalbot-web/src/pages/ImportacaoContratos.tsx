...
export default function ImportacaoContratos() {
  ...
  return (
    <div className="space-y-6">
      <ToastView />
      <div>
        <h1 className="text-2xl font-semibold">Incluir arquivo no banco</h1>
        <p className="text-sm text-slate-600">Envie planilhas XLS ou XLSX para limpar, padronizar e gravar contratos e atas no banco de dados.</p>
      </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div
          className={`flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed p-6 text-center transition ${
            dragging ? 'border-[#1D4ED8] bg-blue-50' : 'border-slate-300 bg-white'
          }`}
          onDragOver={(event) => {
        ...
        >
          ...
          />
          <label htmlFor="contracts-upload" className="cursor-pointer text-[#1D4ED8]">
        ...
        {progress > 0 && (
          <div className="mt-4">
            ...
            <p className="mt-1 text-xs text-slate-600">{progress === 100 ? 'Concluido' : 'Processando planilha...'}</p>
          </div>
        )}

        <div className="mt-4 flex justify-end">
          <Button type="button" onClick={onSubmit} disabled={uploading}>
            {uploading ? 'Importando...' : 'Incluir no banco'}
          </Button>
        </div>
      </section>
      ...
      {result && (
        <section className="space-y-4">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            <h2 className="text-sm font-medium text-slate-700">Erros e linhas ignoradas</h2>
