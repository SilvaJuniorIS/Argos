...
if not exist "frontend\fiscalbot-web\.env" (
    if exist "frontend\fiscalbot-web\.env.example" (
        copy /Y "frontend\fiscalbot-web\.env.example" "frontend\fiscalbot-web\.env" >nul
        echo [OK] frontend\fiscalbot-web\.env criado
    )
)
...
if not exist "frontend\fiscalbot-web\node_modules" (
    echo       Instalando dependencias npm (primeira vez)...
    pushd frontend\fiscalbot-web
    call npm install
    if errorlevel 1 (
        popd
...
start "ARGOS - Frontend" cmd /k "cd /d "%~dp0frontend\fiscalbot-web" && title ARGOS Frontend && npm run dev"
...
echo.
echo   Login admin:
echo     E-mail: admin@teste-real.com.br
echo     Senha:  argos123
echo.
